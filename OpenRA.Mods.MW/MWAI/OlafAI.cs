...
namespace OpenRA.Mods.MW.AI
{
    ...
    public sealed class OlafAIInfo : IBotInfo, ITraitInfo
    {
        public class UnitCategories
        {
            ...
            public readonly HashSet<string> ExcludeFromDefenders = new HashSet<string>();
        }
        ...
        public class BuildingCategories
        {
            ...
            public readonly HashSet<string> SiegeWorkshops = new HashSet<string>();
        ...
        [Desc("ResourceTypes used for Refinerie placing.")]
        public readonly HashSet<string> OrePatches = new HashSet<string>();

        [Desc("Minimum number of peasants the AI should try to maintain before building extra army.")]
    ...
    public sealed class OlafAI : ITick, IBot, INotifyDamage
    {
        ...
        public readonly OlafAIInfo Info;
        ...
        List<MWBaseBuilder> builders = new List<MWBaseBuilder>();
        ...
        public void Activate(Player p)
        {
            ...
            {
                if (Info.FarmFields.Contains(t.Type))
                {
        ...
        {
            // Require at least one Farms, unless we can't build it.
            return CountUndeadBuildingByCommonName(Info.UndeadCommonNames.Crypts, Player);
        }
        ...
        public CPos? ChooseBuildLocation(string actorType, bool distanceToBaseIsImportant, BuildingType type)
        {
            ...
            switch (type)
            {
                ...
                case BuildingType.Farm:
                    ...
                    // Try and place the refinery near a resource field
                    var newnearbyResources = Map.FindTilesInAnnulus(baseCenter, Info.MinBaseRadius, Info.MaxBaseRadius)
                        .Where(a => resourceTypeIndices.Get(Map.GetTerrainIndex(a)))
                        .Shuffle(Random);

                    foreach (var r in newnearbyResources)
                    {
                        var found = findPos(baseCenter, r, Info.MinBaseRadius, Info.MaxBaseRadius);
                        if (found != null)
                            return found;
                ...
                case BuildingType.Regular:
                    ...
                    return findPos(baseCenter, baseCenter, Info.MinBaseRadius, Info.OuterRadiusEnd);
                ...
                case BuildingType.Lumber:
                    ...
                    if (closeTrees.Any())
                    {
                        ...
                        Log.Write("debug", "Kiwi Found position: " + perfectposition);
                    }
            ...
            if (AcolyteHarvester.Any())
            {
                ...
                if (FarmFields != null && FarmFields.Any())
                {
                    ...
                    foreach (var scarecrow in closestfields)
                    {
                        ...
                        if (NumberOfFullCells < Info.MinimumResourceFields) // We want atleast 5 cells filled with resources
                            continue;
        ...
        void CheckAllPatchesForProfit()
        {
            // Find Empty Patches and retreat Acolytes from them:

            var FarmFields = World.ActorsHavingTrait<ISeedableResource>().Where(a => (Info.UndeadCommonNames.PrayableCrops.Contains(a.Info.Name) || Info.UndeadCommonNames.PrayableIron.Contains(a.Info.Name) || Info.UndeadCommonNames.PrayableDeerStands.Contains(a.Info.Name))).ToHashSet();
            var reslay = Player.World.WorldActor.Trait<ResourceLayer>();
            ...
            foreach (var crow in FarmFields)
            {
                ...
                if (doihavepreyers.Count() <= Info.MAxAcolytesOnEmptyPatch)
                {
                ...
                if (getResourceArroundit >= Info.MinimumResourceFields) // We want atleast MinimumResourceFields cells filled with resources
                {
                ...
                {
                    if (AllowedNumber >= Info.MAxAcolytesOnEmptyPatch && dock.Reserver.Owner == Player)
                    {
