...
{
    public class CurseOnHitWarhead : Warhead
    {
        [FieldLoader.Require]
        ...
        public override void DoImpact(Target target, Actor firedBy, IEnumerable<int> damageModifiers)
        {
            ...
            if (actors.Any())
            {
                foreach (var actor in actors)
                {
                    ...
                    {
                        actor.TraitOrDefault<SpawnActorOnCurse>().Cusserplayer = firedBy.Owner;

                        if (!IsValidAgainst(actor, firedBy))
                            continue;

                        var external = actor.TraitsImplementing<ExternalCondition>()
                            .FirstOrDefault(t => t.Info.Condition == Condition && t.CanGrantCondition(actor, firedBy));

                        if (external != null)
                            external.GrantCondition(actor, firedBy, Duration);
                    }
                }
            }
        }
