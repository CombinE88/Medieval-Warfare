...
namespace OpenRA.Mods.MW.Traits.BuildingTraits
{
	...
	public class UpgradeProductionQueueInfo : ITraitInfo, Requires<ConditionManagerInfo>
	{
		[FieldLoader.Require]
		...
		public readonly int ItemLimit = 1;
		...
		public virtual object Create(ActorInitializer init) { return new UpgradeProductionQueue(init, init.Self.Owner.PlayerActor, this); }
	...
	public class UpgradeProductionQueue : IResolveOrder, ITick, ITechTreeElement, INotifyOwnerChanged, INotifyKilled, INotifySold, ISync, INotifyTransform, INotifyCreated
	{
		public readonly UpgradeProductionQueueInfo Info;
		readonly Actor self;

		// A list of things we could possibly build
		protected readonly Dictionary<ActorInfo, ProductionState> Producible = new Dictionary<ActorInfo, ProductionState>();
		protected readonly List<UpgradeProductionItem> Queue = new List<UpgradeProductionItem>();
		...
		public UpgradeProductionQueue(ActorInitializer init, Actor playerActor, UpgradeProductionQueueInfo info)
		{
			self = init.Self;
			Info = info;
		...
		void CacheProducibles(Actor playerActor)
		{
			...
			{
				var bi = a.TraitInfo<UpgradeBuildableInfo>();
		...
		IEnumerable<ActorInfo> AllBuildables(string category)
		{
			return self.World.Map.Rules.Actors.Values
				.Where(x =>
					x.Name[0] != '^' &&
					x.HasTraitInfo<UpgradeBuildableInfo>() &&
					x.TraitInfo<UpgradeBuildableInfo>().Queue.Contains(category));
		}
		...
		public virtual bool IsProducing(UpgradeProductionItem item)
		{
			return Queue.Count > 0 && Queue[0] == item;
		}

		public virtual IEnumerable<UpgradeProductionItem> AllQueued()
		{
			return Queue;
		}
		...
		public virtual IEnumerable<ActorInfo> BuildableItems()
		{
			...
			var buildableProduciblesTemp = buildableProducibles.Where(ai =>
			{
				var bi = ai.TraitInfoOrDefault<UpgradeBuildableInfo>();

				if (bi == null)
					return false;

				foreach (var req in bi.TypeRequirements)
				{
					var rgx = new Regex("[^0-9]");
					var regV = rgx.Replace(req.Value, "");

					int getNum;
					int.TryParse(regV, out getNum);

					var getOperator = req.Value.Remove(getNum);

					if (getOperator == "<" && !(Types.Count(i => i == req.Key) < getNum))
						return false;
			...
			return buildableProduciblesTemp;
		}
		...
		public bool CanQueue(ActorInfo actor, out string notificationAudio)
		{
			...
			var bi = actor.TraitInfoOrDefault<UpgradeBuildableInfo>();
			if (bi == null)
				return false;
			...
			if (!developerMode.AllTech)
			{
				...
				if (bi.BuildLimit > 0)
				{
					var owned = self.Owner.World.ActorsHavingTrait<UpgradeBuildable>()
		...
		public void ResolveOrder(Actor self, Order order)
		{
			...
			switch (order.OrderString)
			{
				case "StartProduction":
					...
					var bi = unit.TraitInfo<UpgradeBuildableInfo>();
					...
					for (var n = 0; n < amountToBuild; n++)
					{
						...
						BeginProduction(new UpgradeProductionItem(this, order.TargetString, cost, playerPower, conditionManager, () => self.World.AddFrameEndTask(_ =>
						{
		...
		public virtual int GetBuildTime(ActorInfo unit, UpgradeBuildableInfo bi)
		{
			...
			if (item != null)
			{
				if (item.Infinite)
				{
					...
					for (var i = 1; i < Info.InfiniteBuildLimit; i++)
						Queue.Add(new UpgradeProductionItem(this, item.Item, item.TotalCost, playerPower, conditionManager, item.OnComplete));
				}
				else
				{
		...
		public void EndProduction(UpgradeProductionItem item)
		{
			if (!self.IsDead && self.IsInWorld && item.ConditionToBuild != ConditionManager.InvalidConditionToken)
			{
				conditionManager.RevokeCondition(self, item.ConditionToBuild);
				item.ConditionToBuild = ConditionManager.InvalidConditionToken;
			}

			Queue.Remove(item);

			if (item.Infinite)
				Queue.Add(new UpgradeProductionItem(this, item.Item, item.TotalCost, playerPower, conditionManager, item.OnComplete) { Infinite = true });
		}

		protected virtual void BeginProduction(UpgradeProductionItem item, bool hasPriority)
		{
			...
			{
				if (item.ConditionToBuild == ConditionManager.InvalidConditionToken)
					item.ConditionToBuild = conditionManager.GrantCondition(self, item.Bi.ConditionWhileBuild);
				Queue.Add(item);
			}
		...
		public virtual int RemainingTimeActual(UpgradeProductionItem item)
		{
		...
		protected virtual bool BuildUnit(ActorInfo unit)
		{
			...
			if (item != null && item.ConditionOnComplete == ConditionManager.InvalidConditionToken)
				item.ConditionOnComplete = conditionManager.GrantCondition(self, item.Bi.ConditionOnFinish);

			if (item != null)
				Types.Add(item.Bi.BuildType);
	...
	public class UpgradeProductionItem
	{
		public readonly string Item;
		public readonly UpgradeProductionQueue Queue;
		public readonly int TotalCost;
		public readonly Action OnComplete;
		public readonly UpgradeBuildableInfo Bi;

		public int ConditionOnComplete = ConditionManager.InvalidConditionToken;
		public int ConditionWhileBuilding = ConditionManager.InvalidConditionToken;
		public int ConditionToBuild = ConditionManager.InvalidConditionToken;
		...
		public UpgradeProductionItem(UpgradeProductionQueue queue, string item, int cost, PowerManager pm, ConditionManager conditionManager, Action onComplete)
		{
			Item = item;
			...
			Bi = ai.TraitInfo<UpgradeBuildableInfo>();
			Infinite = false;
		}
