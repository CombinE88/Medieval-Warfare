...
{
	public class MWProductionTab
	{
		public string Name;
		public ProductionQueue Queue;
	}

	public class MWProductionTabGroup
	{
	...
	public class MWProductionTabsWidget : Widget
	{
		...
		public readonly Dictionary<string, MWProductionTabGroup> Groups;
		...
		Lazy<MWProductionPaletteWidget> paletteWidget;
		string queueGroup;

		[ObjectCreator.UseCtor]
		public MWProductionTabsWidget(World world)
		{
			this.world = world;

			Groups = world.Map.Rules.Actors.Values.SelectMany(a => a.TraitInfos<ProductionQueueInfo>())
				.Select(q => q.Group).Distinct().ToDictionary(g => g, g => new MWProductionTabGroup() { Group = g });

			// Only visible if the production palette has icons to display
			IsVisible = () => queueGroup != null && Groups[queueGroup].Tabs.Count > 0;

			paletteWidget = Exts.Lazy(() => Ui.Root.Get<MWProductionPaletteWidget>(PaletteWidget));
		}
