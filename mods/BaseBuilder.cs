	...
	class BaseBuilder
	{
		...
		ActorInfo GetProducibleBuilding(HashSet<string> actors, IEnumerable<ActorInfo> buildables, Func<ActorInfo, int> orderBy = null)
		{
			var available = buildables.Where(actor =>
			{
				...
				return playerBuildings.Count(a => a.Info.Name == actor.Name || a.Info.Name == actor.Name.Replace(".scaff","") || a.Info.Name == actor.Name.Replace(".PENTA","")) <= ai.Info.BuildingLimits[actor.Name];
			});
		...
		ActorInfo ChooseBuildingToBuild(ProductionQueue queue)
		{
			...
			// Check for not building to much stuff		
			var checkallscaff = playerBuildings.Count(a => a.Info.Name.Contains(".scaff") ||a.Info.Name.Contains(".PENTA"));
			if (checkallscaff >= 2)
			{
				HackyAI.BotDebug("To much scaffolds, build nothing");
				return null;
			}
			
			var checkpeasants = world.ActorsHavingTrait<Targetable>().Where(a => a.Owner == player && a.TraitsImplementing<Targetable>().FirstOrDefault().Info.TargetTypes.Contains("Peasant")).ToArray();
			var consumer = world.ActorsHavingTrait<Mobile>().Where(a => a.Owner == player && !a.TraitsImplementing<Targetable>().FirstOrDefault().Info.TargetTypes.Contains("Peasant")).ToArray();
			
			var freebeds = 0;
			freebeds += playerBuildings.Count(a => a.Info.Name.Contains("nukenew"))*4;
			freebeds += playerBuildings.Count(a => a.Info.Name.Contains("xltnt1"))*8;
			freebeds += playerBuildings.Count(a => a.Info.Name.Contains("xltnt2"))*8;
			freebeds += playerBuildings.Count(a => a.Info.Name.Contains("xltnt3"))*8;
			freebeds += playerBuildings.Count(a => a.Info.Name.Contains("xltntuni"))*8;
			freebeds += playerBuildings.Count(a => a.Info.Name.Contains("factnew"))*6;
			freebeds += playerBuildings.Count(a => a.Info.Name.Contains("nukxl"))*15;
			freebeds += playerBuildings.Count(a => a.Info.Name.Contains("nuk2new"))*8;
			...
			HackyAI.BotDebug("AI: Has {0} Free beds and {1} peasants. {2}/8 desired.", freebeds - consumer.Length,checkpeasants.Length, check);
			
			// First priority is to get out of a low power situation
			// TODO: changed to peasant sotuation
			
			if (check<8)
			{
				if (power != null)
				{
					HackyAI.BotDebug("AI: {0} decided to build {1}: Priority override (low peasants)", queue.Actor.Owner, power.Name);
					return power;
				}
			}
			...
			foreach (var frac in ai.Info.BuildingFractions.Shuffle(ai.Random))
			{
				...
				var count = playerBuildings.Count(a => a.Info.Name == name) + playerBuildings.Count(b => b.Info.Name+".scaff" == name) + playerBuildings.Count(c => c.Info.Name+".PENTA" == name);
				if (count > frac.Value * playerBuildings.Length)
					continue;
				
				var checksamsescaff = playerBuildings.Count(a => a.Info.Name == name);
				if (checksamsescaff >= 1)
					continue;
