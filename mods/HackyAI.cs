	...
	public sealed class HackyAI : ITick, IBot, INotifyDamage
	{
		...
		int CountBuildingByCommonName(HashSet<string> buildings, Player owner)
		{
			return World.ActorsHavingTrait<Building>()
				.Count(a =>
				{
					var nonscaff = a.Info.Name.Replace(".scaff", "");
					var extrascaff = a.Info.Name + ".scaff";
					var nonpenta = a.Info.Name.Replace(".PENTA", "");
					var extrapenta = a.Info.Name + ".PENTA";
						
					if (a.Owner != owner)
						return false;
					
					if (buildings.Contains(a.Info.Name) || buildings.Contains(nonscaff) || buildings.Contains(extrascaff) || buildings.Contains(nonpenta)|| buildings.Contains(extrapenta))
						return true;
